...
describe("TextBufferView", () => {
  ...
  describe("lineInfo getter with wrapping", () => {
    ...
    it("should return byte offsets for no-wrap multi-byte lines", () => {
      ...
      const lineInfo = view.lineInfo
      expect(lineInfo.lineStartBytes).toEqual([0, 7])
    })
    ...
    it("should return byte line starts for #609 multi-byte wrap vectors", () => {
      ...
      for (const vector of vectors) {
        ...
        const lineInfo = view.lineInfo
        expect(lineInfo.lineStartBytes).toEqual(vector.starts)

        const bytes = encoder.encode(vector.text)
        const actualLines = lineInfo.lineStartBytes.map((start, idx) => {
          const end = idx + 1 < lineInfo.lineStartBytes.length ? lineInfo.lineStartBytes[idx + 1] : bytes.length
    ...
    it("should keep byte starts aligned when first grapheme exceeds wrap width", () => {
      ...
      const lineInfo = view.lineInfo
      expect(lineInfo.lineStartBytes).toEqual([0, 3])
      expect(lineInfo.lineWidthCols).toEqual([2, 1])

      const bytes = encoder.encode("가a")
      const lines = lineInfo.lineStartBytes.map((start, idx) => {
        const end = idx + 1 < lineInfo.lineStartBytes.length ? lineInfo.lineStartBytes[idx + 1] : bytes.length
        return decoder.decode(bytes.slice(start, end))
      })
    ...
    it("should return byte line starts for multi-byte word wrapping", () => {
      ...
      const lineInfo = view.lineInfo
      expect(lineInfo.lineStartBytes).toEqual([0, 7])
      expect(lineInfo.lineWidthCols).toEqual([5, 4])
    })
